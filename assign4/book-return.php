...
<?php
	function returnPurchase($cust_id, $purchase_id, $conn){
		...
		// Then you can delete from the table containing the primary key.
		...
		echo "<script> alert('Return completed.'); </script>";
	}
	if ($_SERVER['REQUEST_METHOD'] === "POST"){
		$customer_id = $_POST["customer-id"];
		$purchase_id = $_POST["purchase-id"];
		
		returnPurchase($customer_id, $purchase_id, $conn);
	}
?>
	<script>
		function confirmReturn(){
			$res = confirm("You are about to return this purchase. Are you really sure you want to do this?");
			if (!$res)
	...
	<div class='form-box'>
		<h1 class='center'>Return a Purchase</h1>
		<form action='book-return.php' method='post' onsubmit='return confirmReturn()'>
			<div class="form-item">
